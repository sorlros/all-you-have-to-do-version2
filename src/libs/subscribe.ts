...
// 서버로 구독 정보 전송
export function sendSubscriptionToServer(subscription: any) {
  // 서버로 전송할 데이터 준비
  const requestData = {
  ...
  // 서버 엔드포인트 URL
  const serverEndpoint = `https://fcm.googleapis.com/fcm/send/${process.env.NEXT_PUBLIC_FCM_SERVER_KEY}`;
